...
public class SuppliersController : ControllerBase
{
    ...
    public async Task<IActionResult> PostSupplier(Supplier supplier)
    {
        if (!ModelState.IsValid)
            return BadRequest(new { Message = "Error from model", ModelState });

        var validationContext = new ValidationContext(supplier);
        var validationResult = new List<ValidationResult>();

        if (!Validator.TryValidateObject(supplier, validationContext, validationResult, true))
            return BadRequest(new {Message = "Custom Error", });
    ...
    public async Task<ActionResult<Supplier>> DeleteSupplier(Guid id)
    {
        ...
        {
            if (string.Compare(e.Message, "Not found", StringComparison.OrdinalIgnoreCase) == 0)
